...
def __initialize__():
    ...
    supports_power_consumption = config.get(
        "supports_power_consumption", False)

    if supports_power_consumption:
        subscription_uid = "wirehome.tasmota.relay.ps:" + \
            wirehome.context["component_uid"]

        wirehome.mqtt.subscribe(
            subscription_uid, "stat/" + device_uid + "/STATUS8", __power_consumption_received__)

        update_interval = config.get("power_consumption_update_interval", 5000)
        timer_uid = "wirehome.tasmota.relay.pc.timer." + device_uid
        wirehome.scheduler.start_timer(
            timer_uid, update_interval, __request_power_consumption__)

    subscription_uid = "wirehome.tasmota.relay.stat:" + \
        wirehome.context["component_uid"]
...
def __power_consumption_received__(message):
    ...
    if "StatusSNS" in jsonPayload:
        if "ENERGY" in jsonPayload["StatusSNS"]:
            if "Power" in jsonPayload["StatusSNS"]["ENERGY"]:
                consumption = jsonPayload["StatusSNS"]["ENERGY"]["Power"]
