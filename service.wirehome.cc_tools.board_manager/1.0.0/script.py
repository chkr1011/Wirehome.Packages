...
def stop():
    ...
    if message_bus_subscription != None:
        message_bus.unsubscribe(message_bus_subscription)
...
def __write_pcf8574__(device):
    i2c.write_as_ulong(device.bus_id, device.address, device.buffer, 1)
...
def __write_max7311__(device):
    ...
    buffer = [0x6, 0x0, 0x0]
    i2c.write(device.bus_id, device.address, buffer)
    ...
    i2c.write_as_ulong(device.bus_id, device.address, buffer, 3)
...
def __initialize_device__(device_uid, config):
    ...
    if device.fetch_mode == "interrupt":
        ...
        if device.interrupt_gpio_id != None:
            gpio.set_direction(device.interrupt_gpio_host_id, device.interrupt_gpio_id, "in")
